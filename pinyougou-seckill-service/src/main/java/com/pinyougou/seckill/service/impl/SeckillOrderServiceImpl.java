...
public class SeckillOrderServiceImpl implements SeckillOrderService {
	 ...
	 * 
	 * @see
	 * com.pinyougou.seckill.service.SeckillOrderService#submitOrder(java.lang.Long,
	 * java.lang.String)
	 */
	@Override
	public void submitOrder(Long seckillId, String userId) {
		...
		if (seckillGoods.getStockCount() <= 0) {
			throw new RuntimeException("商品已被洗劫一空");
		}

		// 2.商品库存量减1
		seckillGoods.setStockCount(seckillGoods.getStockCount() - 1);
		// 2.1判断该商品的库存数量
		if (seckillGoods.getStockCount().intValue() == 0) { // 如果库存数量为0 更新数据库 清除缓存
		...
		seckillOrder.setId(seckillOrderId); // 订单ID
		...
		seckillOrder.setStatus("0"); // 订单状态 0未支付 1已支付
	...
	public void saveOrderFromRedisToDb(String userId, Long orderId, String transactionId) {
		...
		seckillOrder.setStatus("1");  //订单状态 0未支付 1已支付
		seckillOrder.setTransactionId(transactionId); //订单流水号
		seckillOrderMapper.insert(seckillOrder);//保存到数据库
		redisTemplate.boundHashOps("seckillOrder").delete(userId); //清楚缓存
	}
